...
function DraggableRow<T>({ row }: { row: Row<T> }) {
  const { transform, transition, setNodeRef, isDragging } = useSortable({
    id: row.original && (row.original as any).id,
  })
  return (
    <TableRow
...
export function DataTable<T extends { id: UniqueIdentifier }>({
  ...
  const filteredData = React.useMemo(() => {
    ...
    return tab && tab.filter ? data.filter(tab.filter) : data;
  }, [data, tabs, activeTab]);
  ...
  if (!tabs) {
    return (
      <div className="w-full flex-col justify-start gap-6">
        {title && <h2 className="text-lg font-bold px-4 pt-4">{title}</h2>}
        {/* Table UI only, no tabs */}
  ...
  return (
    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex-col justify-start gap-6">
      ...
      {tabs.map(tab => (
        <TabsContent key={tab.value} value={tab.value} className="relative flex flex-col gap-4 overflow-auto px-4 lg:px-6">
          <div className="overflow-hidden rounded-lg border">
            <DndContext
              collisionDetection={closestCenter}
              modifiers={[restrictToVerticalAxis]}
