      ...
      <SidebarInset>
        ...
        <div className="space-y-6 px-6 py-4">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">Wallet not found.</div>
          ) : (
            <>
              {/* Stats */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card className="p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-xs font-semibold text-green-700 dark:text-green-400 uppercase tracking-wide">Available Balance</p>
                      <p className="text-2xl font-bold text-green-600 mt-2">₦{wallet.balance.toLocaleString()}</p>
                    </div>
                ...
                <Card className="p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-xs font-semibold text-amber-700 dark:text-amber-400 uppercase tracking-wide">Escrow (Reserved)</p>
                      <p className="text-2xl font-bold text-amber-600 mt-2">₦{wallet.escrow_balance.toLocaleString()}</p>
                    </div>
                ...
                <Card className="p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold mt-2">₦{(wallet.balance + wallet.escrow_balance).toLocaleString()}</p>
                    </div>
              ...
              <Card className="p-4 space-y-3">
                <h3 className="font-semibold flex items-center gap-2"><Wallet className="w-4 h-4" /> Wallet Information</h3>
                <Separator />
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-mono font-semibold mt-1">{wallet.user_id}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground uppercase">Created</p>
                    <p className="font-semibold mt-1">{new Date(wallet.created_at).toLocaleDateString()}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground uppercase">Last Updated</p>
                    <p className="font-semibold mt-1">{wallet.updated_at ? new Date(wallet.updated_at).toLocaleDateString() : "N/A"}</p>
                  </div>
                </div>
              </Card>
              ...
              {/* Transactions */}
              <Card className="p-4 space-y-3">
                <h3 className="font-semibold">Transaction History ({wallet.transactions.length})</h3>
                <Separator />
