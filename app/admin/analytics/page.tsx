...
import {
  ...
  AnalyticsInterval, VendorOrderType,
...
function OrderTrendsChart() {
  ...
  const { data, isLoading } = useQuery<OrderTrendPoint[]>({
    ...
    queryFn: () => fetchApi(`/api/analytics?endpoint=order-trends&days=${days}&interval=${interval}`),
  })
    ...
    <Card>
      <CardHeader className="flex flex-row items-start justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Select value={interval} onValueChange={(v) => setInterval(v as AnalyticsInterval)}>
          ...
          <ChartContainer config={orderTrendConfig} className="h-64 w-full">
            <AreaChart data={data}>
              ...
              {Object.entries(orderTrendConfig).map(([key, cfg]) => (
                <Area key={key} type="monotone" dataKey={key} stroke={cfg.color} fill={cfg.color} fillOpacity={0.15} strokeWidth={2} dot={false} />
              ))}
            </AreaChart>
          </ChartContainer>
        )}
...
function UserGrowthChart() {
  ...
  const { data, isLoading } = useQuery<UserGrowthPoint[]>({
    ...
    queryFn: () => fetchApi(`/api/analytics?endpoint=user-growth&days=${days}&interval=${interval}`),
  })
          ...
          <ChartContainer config={userGrowthConfig} className="h-64 w-full">
            <BarChart data={data}>
              ...
              {Object.entries(userGrowthConfig).map(([key, cfg]) => (
                <Bar key={key} dataKey={key} stackId="a" fill={cfg.color} radius={key === 'laundry_vendors' ? [4, 4, 0, 0] : [0, 0, 0, 0]} />
              ))}
            </BarChart>
          </ChartContainer>
        )}
...
// ── Status Breakdown ──────────────────────────────────────────
function StatusPie({ title, data }: { title: string; data: { status: string; count: number; percentage: number }[] }) {
  return (
...
function StatusBreakdownSection() {
  ...
  const { data, isLoading } = useQuery<StatusBreakdownResponse>({
    ...
    queryFn: () => fetchApi(`/api/analytics?endpoint=status-breakdown&days=${days}`),
  })
    ...
    <Card>
      <CardHeader className="flex flex-row items-start justify-between">
        <div>
          ...
          <CardDescription>Order status distribution by service</CardDescription>
        </div>
        <Select value={days} onValueChange={setDays}>
          <SelectTrigger className="h-8 w-28 text-xs"><SelectValue /></SelectTrigger>
          <SelectContent>{DAYS_OPTIONS.map(o => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}</SelectContent>
...
function TransactionSection() {
  ...
  const { data, isLoading } = useQuery<TransactionAnalyticsResponse>({
    ...
    queryFn: () => fetchApi(`/api/analytics?endpoint=transactions&days=${days}&interval=${interval}`),
  })
          ...
          <>
            <ChartContainer config={txTrendConfig} className="h-56 w-full">
              <AreaChart data={data.trend}>
                ...
                <Area yAxisId="right" type="monotone" dataKey="count" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.15} strokeWidth={2} dot={false} />
              </AreaChart>
            </ChartContainer>
                ...
                <div className="space-y-1">
                  {data.by_type.map((t) => (
                    <div key={t.type} className="flex justify-between text-xs">
                      ...
                      <span className="font-mono">{fmt(t.volume)} ({t.count})</span>
                    </div>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="space-y-1">
                  {data.by_order_type.map((t) => (
                    <div key={t.order_type} className="flex justify-between text-xs">
...
function TopRidersSection() {
  ...
  const { data, isLoading } = useQuery<TopRider[]>({
    ...
    queryFn: () => fetchApi(`/api/analytics?endpoint=top-riders&days=${days}&limit=10`),
  })
            ...
            {data?.map((rider, i) => (
              <div key={rider.id} className="flex items-center gap-3">
                ...
                <Avatar className="w-8 h-8">
                  ...
                  <AvatarFallback className="text-xs">{rider.full_name?.[0] ?? '?'}</AvatarFallback>
                </Avatar>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{rider.full_name ?? 'Unknown'}</p>
                  <p className="text-xs text-muted-foreground">{rider.bike_number ?? '—'} · {rider.dispatcher_business_name ?? 'Independent'}</p>
                </div>
...
function TopVendorsSection() {
  ...
  const { data, isLoading } = useQuery<TopVendor[]>({
    ...
    queryFn: () => fetchApi(`/api/analytics?endpoint=top-vendors&days=${days}&limit=10&order_type=${orderType}`),
  })
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-start justify-between">
        ...
        <div className="flex gap-2">
          <Select value={orderType} onValueChange={(v) => setOrderType(v as VendorOrderType)}>
            <SelectTrigger className="h-8 w-28 text-xs"><SelectValue /></SelectTrigger>
            <SelectContent>
          ...
          <div className="space-y-3">
            {data?.map((vendor, i) => (
              <div key={vendor.id} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{vendor.name ?? 'Unknown'}</p>
                  <p className="text-xs text-muted-foreground">{vendor.total_orders} total · {vendor.cancelled_orders} cancelled</p>
                </div>
                <div className="text-right shrink-0 space-y-0.5">
