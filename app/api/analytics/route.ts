...
async function proxyGet(path: string, accessToken: string, params?: URLSearchParams) {
  ...
  const base = path ? `/analytics/${path}` : "/analytics";
  const url = `${process.env.API_URL}${base}${params?.toString() ? `?${params}` : ""}`;
...
export async function GET(request: NextRequest) {
  ...
  const endpoint = sp.has("endpoint") ? sp.get("endpoint") : "overview";
